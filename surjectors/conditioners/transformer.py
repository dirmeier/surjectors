...
def transformer_conditioner(
        output_size, num_heads=2, num_layers=2, key_size=32, dropout_rate=0.1, widening_factor=4
):
    linear = hk.Linear(key_size * num_heads)
    encoder = _EncoderLayer(
      num_heads, num_layers, key_size, dropout_rate, widening_factor
    )
    transformer = _AutoregressiveTransformerEncoder(linear, encoder, output_size * 2)
    return transformer
